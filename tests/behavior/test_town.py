...
def test_gold_is_deposited_and_verified_by_the_balance(town):
    ...
    # Above the 30000 restock reserve, so it deposits; smaller amounts are
    # kept on hand for Akara (tested in test_gold_below_reserve_is_kept).
    town.gold, town.gold_stash = 42435, 805539
    full_belt(town)
    town.inventory = [loot(1, (0, 0))]

    report = PreambleReport()
    layer(town).manage_inventory(report)
    assert town.gold == 0 and town.gold_stash == 847974
    assert any("gold: 42435 deposited" in line for line in report.log)


def test_gold_below_reserve_is_kept_for_restock(town):
    """R241: a level-92 necro banking every coin left the Akara restock
    with nothing to spend. Below the reserve, gold stays on the character."""
    ...
    report = PreambleReport()
    layer(town).manage_inventory(report)
    assert town.gold == 12435 and town.gold_stash == 805539  # nothing banked
    assert any("kept as restock reserve" in line for line in report.log)
...
def test_a_missed_gold_click_leaves_no_chat_console_behind(town):
    ...
    town.gold, town.gold_stash = 35000, 1000  # above the reserve, so it deposits
    full_belt(town)
    town.inventory = [loot(1, (0, 0))]
    ...
    assert town.gold == 35000  # nothing moved
