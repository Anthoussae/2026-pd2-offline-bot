...
- **P1–P5** (per the phase files): config trio; route line + leash
  ...
  mandatory pickup (the order book: open → resight-through-combat →
  id-churn rebind → expiry → full-inventory → queued/mid-field cleanse
  → budget close), piloted live and census-gated.
- **P6** (this closeout): `pickup.order_book_diag` scaffolding removed;
  ...
  three tagmode-review P3s FIXED with tests (001 `registered_say` —
...
- **Detour 3 — the locomotion speed pass** (R255–R262, T92/T93,
  archived `2026-08-13-clear-radius-locomotion`): Cold Plains 454 s →
  ~150–180 s typical. The human benchmark method (`pd2bot.observe` +
...
Suite grew 1221 → **1349 tests, green**, ruff clean throughout; every
...
eleven T93 acceptance launches, one human benchmark recording —
...
archived record) — the plan's shape held around them. The tag-mode
...
watch; waypoint SW aim; ring 537 deposit retry; `field_at` → `cast_at`
