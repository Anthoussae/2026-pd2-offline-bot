...
ticks > 4 s â€” solved outright in every run since P2); the idle < 45 s
target closed at 48â€“95 s and was accepted-as-revised with the
remainder queued as future work.

## Completed work

- **P1** â€” `nav.plan` plan-cost telemetry (navigator + route_service,
...
- **P2** â€” bounded A* (`default_node_budget`: floor 2 000 / cap 25 000,
...
- **P3** â€” `default_posture = "berserk"` (R256 QA operator ruling,
  applied at module construction, per-step overrides intact);
  `patrol_step` 12 â†’ 20; `describe()` prints the standing posture.
- **P4** â€” eleven live launches with four operator-approved structural
  fixes shipped mid-battery: **walk-in attacks** (R259 â€” charge
  attacks at range via the client's own unit-aware pathing;
  `AttackUnit.walk_in`), the **seam gate** (R260 â€” no engagement
...
- **P5** â€” performance-notes re-priced with a what-bought-what table;
  navigation.md, behavior.md, run-log.md and the necro module
  docstring updated; ADR written; cleanup grep clean.

## Validation

Full suite grew 1299 â†’ **1345 tests, all green**, ruff clean at every
...
- BUDGET_CAP 50k â†’ 25k and the two same-night reverts, all recorded
